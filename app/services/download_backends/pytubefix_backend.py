...
class PytubefixBackend(DownloadBackend):
    ...
    def _progressive_key(stream) -> tuple[int, int, str]:
        ...
        return (int(resolution) if resolution.isdigit() else 0, int(getattr(stream, "fps", 0) or 0), str(getattr(stream, "itag", "")))
    ...
    async def probe(self, url: str) -> NormalizedMediaResult:
        ...
        def run() -> NormalizedMediaResult:
            ...
            for stream in yt.streams.filter(type="video"):
                ...
                height = int(resolution.removesuffix("p")) if resolution.removesuffix("p").isdigit() else None
                if height:
                    formats.append({"format_id": str(stream.itag), "height": height, "ext": getattr(stream, "subtype", None), "fps": getattr(stream, "fps", None)})
    ...
    async def expand_playlist(self, url: str, *, limit: int | None = None) -> list[dict[str, object]]:
        ...
        def run() -> list[dict[str, object]]:
            ...
            safe_limit = self.settings.max_playlist_items if limit is None else min(limit, self.settings.max_playlist_items)
            if len(urls) > safe_limit:
                raise ValueError(f"Playlist exceeds safe limit of {safe_limit} items")
            return [{"url": item, "title": None, "index": index} for index, item in enumerate(urls, 1)]
        return await asyncio.to_thread(run)
    ...
    async def download(self, request: DownloadRequest) -> NormalizedMediaResult:
        ...
        def download_streams() -> tuple[Path, Path | None]:
            ...
            if request.media_type == "audio":
                audio = sorted(yt.streams.filter(only_audio=True), key=self._audio_key, reverse=True)
                if not audio:
                    raise FormatUnavailableError("pytubefix found no audio stream")
                path = Path(audio[0].download(output_path=str(request.output_dir), filename="pytubefix-audio.m4a", timeout=self.settings.ytdlp_socket_timeout_seconds, max_retries=self.settings.ytdlp_retries, interrupt_checker=interrupted))
                return path, None

            requested = request.quality.lower().strip().removesuffix("p")
            progressive = sorted(yt.streams.filter(progressive=True, file_extension="mp4"), key=self._progressive_key, reverse=True)
            if requested != "best":
                progressive = [s for s in progressive if str(getattr(s, "resolution", "")).removesuffix("p") == requested]
            if progressive:
                path = Path(progressive[0].download(output_path=str(request.output_dir), filename="pytubefix-video.mp4", timeout=self.settings.ytdlp_socket_timeout_seconds, max_retries=self.settings.ytdlp_retries, interrupt_checker=interrupted))
                return path, None

            adaptive = sorted(yt.streams.filter(only_video=True, file_extension="mp4"), key=self._progressive_key, reverse=True)
            if requested != "best":
                adaptive = [s for s in adaptive if str(getattr(s, "resolution", "")).removesuffix("p") == requested]
            audio = sorted(yt.streams.filter(only_audio=True), key=self._audio_key, reverse=True)
            if not adaptive or not audio:
                raise FormatUnavailableError("pytubefix found no deterministic stream combination")
            video_path = Path(adaptive[0].download(output_path=str(request.output_dir), filename="pytubefix-video-only.mp4", timeout=self.settings.ytdlp_socket_timeout_seconds, max_retries=self.settings.ytdlp_retries, interrupt_checker=interrupted))
            audio_path = Path(audio[0].download(output_path=str(request.output_dir), filename="pytubefix-audio-only.m4a", timeout=self.settings.ytdlp_socket_timeout_seconds, max_retries=self.settings.ytdlp_retries, interrupt_checker=interrupted))
        ...
        if secondary is not None:
            ...
            await _run_process(
                ...
                "-i", str(video_or_audio), "-i", str(secondary),
            ...
            await _run_process(
                "ffmpeg", "-hide_banner", "-loglevel", "error", "-y", "-i", str(video_or_audio),
