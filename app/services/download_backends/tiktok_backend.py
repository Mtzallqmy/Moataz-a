...
class TikTokBackend(DownloadBackend):
    ...
    async def supports(self, url: str, media_type: str | None = None) -> bool:
        ...
        return host.endswith(("tiktok.com", "douyin.com")) and media_type in {
            None,
            "video",
            "audio",
            "images",
    ...
    async def _request(self, request: DownloadRequest, *, download: bool) -> dict:  # noqa: ARG002
        if not await self.available():
            raise BackendUnavailableError(
                "TikTok backend is disabled or TIKTOK_BACKEND_URL is not configured"
            )
        ...
        async with aiohttp.ClientSession(timeout=timeout) as session:
            async with session.post(
                endpoint, headers=self._headers(), json=self._payload(request)
            ) as response:
                ...
                if response.status >= 400:
                    raise BackendUnavailableError(
                        f"TikTok sidecar rejected request with HTTP {response.status}"
                    )
    ...
    async def download(self, request: DownloadRequest) -> NormalizedMediaResult:
        ...
        try:
            for index, item in enumerate(payload["media"], 1):
                ...
                suffix = {"image": ".jpg", "audio": ".m4a", "video": ".mp4"}.get(
                    str(item.get("kind")), ".bin"
                )
                output = request.output_dir / f"tiktok-{index:03d}{suffix}"
                timeout = aiohttp.ClientTimeout(
                    total=self.settings.tiktok_backend_timeout_seconds
                )
                async with aiohttp.ClientSession(timeout=timeout) as session:
                            ...
                            async for chunk in response.content.iter_chunked(256 * 1024):
                                if (
                                    ...
                                    raise CancelledError(
                                        "TikTok sidecar download cancelled"
                                    )
                                total += len(chunk)
                                if total > self.settings.max_file_size_bytes:
                                    raise RuntimeError(
                                        "TikTok outputs exceed configured download limit"
                                    )
                                handle.write(chunk)
                if suffix in {".mp4", ".m4a"}:
                    await probe_media_file(output)
                elif suffix == ".jpg" and output.read_bytes()[:3] != b"\xff\xd8\xff":
