...
# Patch the existing UX functions once so every legacy entry point receives the upgraded menus.
...
async def _process_split_download(job_id: int, segment_seconds: int) -> None:
    cancel_event = worker_module._cancel_events.setdefault(job_id, __import__("threading").Event())
